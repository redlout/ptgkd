...
export default {
  ...
  handleWill() {
    // 意志 （决心+专注+意志）=5
    if(store.state.fx3.tableData[7].data.length == 0){
      store.state.fx4.investigation =
      store.state.fx2.tableData[5].base.base + store.state.fx3.tableData[7].base;
    }
  ...
  handleReflection() {
    // 反射 （敏捷+运动+反射）= 12
    if(store.state.fx3.tableData[0].data.length == 0){
      store.state.fx4.investigation =
  ...
  handleStrong() {
    // 强韧 （耐力+生存+强韧）= 2
    if(store.state.fx3.tableData[3].data.length == 0){
      store.state.fx4.investigation =
  ...
  handleInvestigation() {
    ...
    for (var i = 0; i < store.state.fx3.tableData[8].data.length; i++) {
      ...
      console.log("侦察" + store.state.fx4.investigation);
      console.log("感知" + store.state.fx2.tableData[4].base);
      console.log("洞察" + store.state.fx3.tableData[8].base);
      console.log("数值" + store.state.fx3.tableData[8].data);
    }
  },
