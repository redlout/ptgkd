  ...
  <div style="width:90%; margin:0 auto">
            ...
            <template slot-scope="scope">
              <span style="width:50%">
                <el-tag
                  ...
                  :type="item.type"
                  effect="plain"
                >{{ item.professional + ": " + item.level }}</el-tag>
              </span>
              <!-- </el-input> -->
              <el-button
                type="primary"
    ...
    <el-dialog :visible.sync="Dialog" center>
      <el-row>
        ...
        <span>{{$store.state.fx3.tableData[0].data}}</span>
        <br />
    ...
    handleEdit(index, row) {
      ...
      if(index <=3){
        if(this.$store.state.fx3.physiological_max>0){
          this.$store.state.fx3.physiological_max -= row.base;
        }else if(this.$store.state.fx3.free_max >0){
          this.$store.state.fx3.free_max -= (row.base-6);
        }else{
          this.diasabledInput = true;
        }
      }else if(index>3 && index <=7){
        if(this.$store.state.fx3.mind_max_max>0){
          this.$store.state.fx3.mind_max_max -= row.base;
        }else if(this.$store.state.fx3.free_max >0){
          this.$store.state.fx3.free_max -= (row.base-5);
        }else{
          this.diasabledInput = true;
        }
      }else{
        if(this.$store.state.fx3.interactive_max>0){
          this.$store.state.fx3.interactive_max -= row.base;
        }else if(this.$store.state.fx3.free_max >0){
          this.$store.state.fx3.free_max -= (row.base-4);
        }else{
          this.diasabledInput = true;
        }
      }
