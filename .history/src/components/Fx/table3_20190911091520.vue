    ...
    <el-dialog :visible.sync="Dialog" center>
      ...
      <el-row>
        <el-col :span="24">
          <el-table
            size="mini"
            :data="professional_table.pro_data"
            border
            style="width: 100%"
...
export default {
  ...
  methods: {
    ...
    readMasterUser(index) {
      this.professional_table.pro_data[index].map(i => {
        i.id = index;
        i.isSet = false;
        return i;
      });
    ...
    //添加项目
    addMasterUser(index) {
      // console.log(this.professional_table.pro_data[index].data)
      // for (let i of this.professional_table.pro_data[index].data) {
      ...
      this.professional_table.pro_data[index].data.push(j);
    ...
    //删除
    msgDelete(row, index) {
      this.professional_table.pro_data[index].data.splice(index, 1);
      this.$message({
        type: "success",
        message: "删除成功!"
      });
