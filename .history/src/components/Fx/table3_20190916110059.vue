...
export default {
  ...
  computed: {
    ...
    ...mapState({
      oldSkillsBase: state => state.fx3.oldSkillsBase
    })
  },
  watch: {
    tableData: {
  ...
  methods: {
    ...
    handleEdit(index, row) {
      ...
      if (oldSkillsBase[index] < row.base) {
        rulejs.inputNumberRule(index, true);//增加
      } else {
        rulejs.inputNumberRule(index, false);//减少
      }
      //更新旧值
      oldSkillsBase[index] = this.$store.state.fx3.tableData[index].base;
      //manage 进行衍生属性
      datajs.manage();
    },
    //修改指定单元格样式
