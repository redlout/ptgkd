...
export default {
  ...
  methods: {
    ...
    handleEdit(index, row){
        ...
        this.$store.state.fx4.life = this.tableData[2].base + this.$store.state.form.volume+this.tableData[2].additional*2;
  ...
  /////////////////使用this.$store
        ...
        for(var i=0 ;i< this.spData[7].data.length;i++){
          if(this.spData[7].data[i].professional == "意志"){
            this.$store.state.fx4.will = this.tableData[5].base + this.spData[7].base + this.spData[7].data[i].level;
            // console.log("no" + this.derivative.will);
          }else{
            this.$store.state.fx4.will = this.tableData[5].base + this.spData[7].base
            // console.log("no" + this.derivative.will);
          }
        }
// 反射 （敏捷【8】+运动+反射【4】）= 12
