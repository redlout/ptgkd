...
export default {
  ...
  methods: {
    ...
    readMasterUser() {
      //根据实际情况，自己改下啊
      this.$data.master_user.data.map(i => {
    ...
    addMasterUser() {
      for (let i of this.$data.master_user.data) {
        if (i.isSet) return this.$data.$message.warning("请先保存当前编辑项");
      }
      let j = {
        id: 0,
        type: "",
      ...
      this.$data.master_user.data.push(j);
      this.$data.master_user.sel = JSON.parse(JSON.stringify(j));
    },
    //修改
    pwdChange(row, index, cg) {
      ...
      for (let i of this.$data.master_user.data) {
        if (i.isSet && i.id != row.id) {
          this.$data.$message.warning("请先保存当前编辑项");
          return false;
        }
      }
      //是否是取消操作
      if (!cg) {
        if (!this.$data.master_user.sel.id) this.$data.master_user.data.splice(index, 1);
      ...
      if (row.isSet) {
        //项目是模拟请求操作  自己修改下
        (function() {
          let data = JSON.parse(JSON.stringify(this.$data.master_user.sel));
          ...
          //然后这边重新读取表格数据
          this.$data.readMasterUser();
          row.isSet = false;
        })();
      } else {
        this.$data.master_user.sel = JSON.parse(JSON.stringify(row));
        row.isSet = true;
      }
    }
  }
