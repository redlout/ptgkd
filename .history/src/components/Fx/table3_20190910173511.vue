            ...
            <template slot-scope="scope">
                <span v-if="scope.row.isSet">
                  ...
                  class="inline-input"
                    ...
                    :fetch-suggestions="querySearch"
                    @select="handleSelect"
                    v-model="tableData.sel[tableData.columns[0].field]"
                  ></el-autocomplete>
                  
                </span>
                <span v-else>{{scope.row[columns[0].field]}}</span>
              </template>
          </el-table-column>

        <!-- <el-table-column 
          :prop="professional_table.columns[1].field" 
          :label="professional_table.columns[1].title"
          :width="professional_table.columns[1].width"
          :key="professional_table.columns[1].id">
            <template slot-scope="scope">
                <span v-if="scope.row.isSet">
                  <el-autocomplete
                  class="inline-input"
                    ...
                    :fetch-suggestions="querySearch"
                    @select="handleSelect"
                    v-model="tableData.sel[professional_table.columns[1].field]"
                  ></el-autocomplete>
                </span>
                <span v-else>{{scope.row[professional_table.columns[1].field]}}</span>
              </template>
          </el-table-column> -->
...
export default {
  ...
  methods: {
    ...
    querySearch(queryString, cb){
      var tb = this.tableData[this.index];
      var results = tb.professional;
      cb(results);
    },
    //建议输入框选中事件
    handleSelect(item) {
    ...
    readMasterUser(index) {
      this.tableData[index].data.map(i => {
        i.id = index; 
        i.isSet = false; 
        return i;
      });
    ...
    //添加项目
    addMasterUser(index) {
      for (let i of this.tableData[index].data) {
      ...
      this.tableData[index].data.push(j);
      this.tableData[index].sel = JSON.parse(JSON.stringify(j));
      console.log("over");
    },
    //修改
    pwdChange(row, index, cg) {
      ...
      if (row.isSet) {
        //项目是模拟请求操作  自己修改下
        let data = this.tableData[index].sel;
        ...
        this.tableData[index].sel = JSON.parse(JSON.stringify(row));
        row.isSet = true;
      }
    },
    //删除
    msgDelete(row, index) {
      this.tableData[index].data.splice(index, 1);
      this.$message({
        type: "success",
        message: "删除成功!"
      });
